---
title: "scRNA-seq Integration and Preparation"
output: rmarkdown::html_vignette
vignette: >
  %/VignetteIndexEntry{scRNA-seq Integration and Preparation}
...
for (i in seq_along(seuratObjList)) {
  # Remove rRNA contamination genes (Gm42418, AY036118 overlap Rn45s)
...
seuratObjList <- lapply(
  X = seuratObjList, FUN = SCTransform, method = "glmGamPoi",
  vars.to.regress = c("percent.mt", "percent.rps", "percent.rpl", "nCount_RNA", "nFeature_RNA"),
  verbose = TRUE
)
...
immune.anchors <- FindIntegrationAnchors(
  object.list = seuratObjList, normalization.method = "SCT",
  anchor.features = features
)

integrated_sctransformed <- IntegrateData(anchorset = immune.anchors, normalization.method = "SCT")

nrow(integrated_sctransformed[["integrated"]]@scale.data)

integrated_sctransformed$INF <- ifelse(
  test = integrated_sctransformed$orig.ident %in% c("SNp1", "SNp2"),
  yes = "naive", no = "infected"
)
```
...
```{r label_transfer_and_phenotyping}
# Manual labelling based on CellMesh (https://uncurl.cs.washington.edu/db_query)
integrated_sctransformed <- SetIdent(integrated_sctransformed, value = "seurat_clusters")
integrated_sctransformed <- RenameIdents(integrated_sctransformed,
  `0` = "Ifng+CD4+T", `1` = "B cells", `2` = "Lyz2hi_MoMac",
  `3` = "Hepatocytes1", `4` = "CD8_T", `5` = "Hepatocytes2",
  `6` = "Prolif. cells", `7` = "Activ_CD4+_T2", `8` = "NK",
  `9` = "CD4_T3", `10` = "Naïve_T", `11` = "ApoeHI_Kupffer",
  `12` = "RBCs1", `13` = "Neutrophils", `14` = "APCs",
  `15` = "RBCs2", `16` = "RBCs3", `17` = "Neutrophils/Mono",
  `18` = "Treg", `19` = "pre-cDCs", `20` = "Mast"
)

# Fine-grained phenotype
integrated_sctransformed$CellMeshFine <- Idents(integrated_sctransformed)

# Coarse-grained phenotype
integrated_sctransformed <- RenameIdents(integrated_sctransformed,
  `Ifng+CD4+T` = "T cells", `B cells` = "B cells",
  `Lyz2hi_MoMac` = "Macrophages", `Hepatocytes1` = "Hepatocytes",
  `CD8_T` = "T cells", `Hepatocytes2` = "Hepatocytes",
  `Prolif. cells` = "Prolif. cells", `Activ_CD4+_T2` = "T cells",
  `NK` = "NK cells", `CD4_T3` = "T cells", `Naïve_T` = "T cells",
  `ApoeHI_Kupffer` = "Macrophages", `RBCs1` = "RBCs",
  `Neutrophils` = "Neutrophils", `APCs` = "Macrophages",
  `RBCs2` = "RBCs", `RBCs3` = "RBCs",
  `Neutrophils/Mono` = "Neutrophils/Mono",
